...
def preprocess(data: Union[np.ndarray, pd.DataFrame], cfg: DictConfig) -> Union[np.ndarray, pd.DataFrame]:
    data_cfg = cfg.data
    if data_cfg.moving_average > 1:
        data = moving_average(data, window=data_cfg.moving_average)
    if data_cfg.logarithm:
        data = log(data)
    if data_cfg.scaling_kind == 'zscore':
        data = scale(data)
    elif data_cfg.scaling_kind == 'minmax':
        data = minmax_scale(data)
    elif data_cfg.scaling_kind == 'normalize':
        data = normalize(data)
    if data_cfg.whitenoise > 0:
        seed = getattr(cfg, 'seed', None)
        data = add_whitenoise(data, std=data_cfg.whitenoise, seed=seed)
